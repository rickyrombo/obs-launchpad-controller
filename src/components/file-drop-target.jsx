...
export default function FileDropTarget({name, value, option, onCellSettingsChanged}) {
  let input;
  ...
  const dragAndDropEvents = {
    ...
    onDrop: async (e) => {
        ...
        if (e.dataTransfer.files.length > 0) {
          ...
          onCellSettingsChanged({[name]: file.name});
  ...
  return <div className={className} {...dragAndDropEvents}>
    <label className="label">{option.label}:
      <input ref={(ref) => input = ref} style={{display: 'none'}} onChange={onChange} type="file" />
    </label>
